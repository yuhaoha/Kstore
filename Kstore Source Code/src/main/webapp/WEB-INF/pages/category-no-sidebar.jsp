...
function quickview(id){
 $.ajax({ 
  ...
  success:function(goods) {
   $("#quickviewname").text(goods.name); 
   $("#quickviewprice").text(goods.price); 
   $("#quickviewpicture").attr("src",goods.pictures); 
   $("#quickviewdescription").text(goods.description); 
   $("#quickviewstate").text(goods.state);
   $("#quickviewtime").text(goods.release_time); 
  }
 });
}
</script>
	...
	<section class="hero">
		<div class="container">
			...
			<div class="hero-content pb-5 text-center">
				<h3 class="hero-heading text-muted">${requestScope.title}</h3>
				<div class="row">
			...
			<div class="products-grid col-12 sidebar-none">
				<header class="product-grid-header">
					<div class="mr-3 mb-3">
						...
						展示 <strong>${goodsList.pages}</strong>页，共<strong>${goodsList.total}
						</strong>个商品
					</div>
					<div class="mr-3 mb-3">
						<span class="mr-2">显示</span>
						<c:if test="${requestScope.shownum==12}">
							<a
								href="${requestScope.page}?shownum=12&search=${requestScope.search}&sortfunc=${requestScope.sortfunc}"
								class="product-grid-header-show active">12 </a>
						</c:if>
						<c:if test="${requestScope.shownum!=12}">
							<a
								href="${requestScope.page}?shownum=12&search=${requestScope.search}&sortfunc=${requestScope.sortfunc}"
						...
						<c:if test="${requestScope.shownum==24}">
							<a
								href="${requestScope.page}?shownum=24&search=${requestScope.search}&sortfunc=${requestScope.sortfunc}"
								class="product-grid-header-show active">24 </a>
						</c:if>
						<c:if test="${requestScope.shownum!=24}">
							<a
								href="${requestScope.page}?shownum=24&search=${requestScope.search}&sortfunc=${requestScope.sortfunc}"
								class="product-grid-header-show">24 </a>
						</c:if>
						<c:if test="${requestScope.shownum==500}">
							<a
								href="${requestScope.page}?shownum=500&search=${requestScope.search}&sortfunc=${requestScope.sortfunc}"
								class="product-grid-header-show active">全部 </a>
						</c:if>
						<c:if test="${requestScope.shownum!=500}">
							<a
								href="${requestScope.page}?shownum=500&search=${requestScope.search}&sortfunc=${requestScope.sortfunc}"
								class="product-grid-header-show">全部 </a>
						</c:if>
					</div>

					<div class="btn-group mb-3 d-flex align-items-center" role="group">
						...
						<div class="dropdown-menu dropdown-menu-right"
							...
							<a class="dropdown-item"
								href="${requestScope.page}?sortfunc=orderby_0&search=${requestScope.search}">默认排序</a>
							<a class="dropdown-item"
								href="${requestScope.page}?sortfunc=orderby_1&search=${requestScope.search}">按发布时间(最新)排序</a>
							<a class="dropdown-item"
								href="${requestScope.page}?sortfunc=orderby_2&search=${requestScope.search}">按价格(由低到高)排序</a>
							<a class="dropdown-item"
								href="${requestScope.page}?sortfunc=orderby_3&search=${requestScope.search}">按价格(由高到低)排序</a>
						</div>
					</div>
				</header>
				<div class="row">
					${requestScope.goodsmessage }
					<!-- product-->
							...
							<div class="product">
								<div class="product-image">
									<c:if test="${goods.state==false}">
										<div class="ribbon ribbon-info">在售</div>
									</c:if>
									<c:if test="${goods.state==true}">
										<div class="ribbon ribbon-success">已售出</div>
									</c:if>
									<img src="${goods.pictures}" alt="product" class=""
										height="380px" width="255px" />
									<div class="product-hover-overlay">
										<a href="detail?detailnum=${goods.goods_id }"
											class="product-hover-overlay-link"></a>
										<div class="product-hover-overlay-buttons">
											<a href="addCollect?detailnum=${goods.goods_id }"
												class="btn btn-outline-dark btn-product-left"><i
												class="fa fa-shopping-cart"></i></a><a
												href="detail?detailnum=${goods.goods_id }"
												class="btn btn-dark btn-buy"><i class="fa-search fa"></i><span
												class="btn-buy-label ml-2">详情</span></a><a
												onclick="quickview(${goods.goods_id})" data-toggle="modal"
					...
					<ul class="pagination">
						<c:if test="${goodsList.hasPreviousPage}">
							<li class="page-item"><a
								href="${requestScope.page}?pn=${goodsList.prePage}&search=${requestScope.search}"
								aria-label="Previous" class="page-link"><span
									aria-hidden="true">上一页</span><span class="sr-only"></span></a></li>
						</c:if>
						<c:forEach items="${goodsList.navigatepageNums}" var="page">
							<c:if test="${page==goodsList.pageNum}">
								<li class="page-item active"><a
									href="${requestScope.page}?pn=${page}&search=${requestScope.search}"
						...
						<c:if test="${goodsList.hasNextPage}">
							<li class="page-item"><a
								href="${requestScope.page}?pn=${goodsList.nextPage}&search=${requestScope.search}"
								aria-label="Next" class="page-link"><span aria-hidden="true">下一页</span><span
		...
		<div role="document" class="modal-dialog modal-lg">
			<div class="modal-content">
				<button type="button" data-dismiss="modal" aria-label="Close"
					class="close modal-close" onclick="window.location.reload(false);">
              ...
              <use xlink:href="#close-1"> </use>
				...
				<div class="modal-body">
					...
					<c:if test="${goods.state==true}">
						<div class="ribbon ribbon-info">已售出</div>
					</c:if>
					<div class="row">
						<div class="col-lg-6">
							<div data-slider-id="1"
								...
								<div style="background-size: cover;"
									...
									<img id="quickviewpicture" class="" height="100%"/>
								</div>
							</div>
						</div>
						<div class="col-lg-6 d-flex align-items-center">
							<div>
								<h2 class="mb-4 mt-4 mt-lg-1">
									<span id="quickviewname"></span>
								</h2>
								<div
									class="d-flex align-items-center justify-content-between mb-4">
									<ul class="list-inline mb-0">
										<li class="list-inline-item h4 font-weight-light mb-0"
											id="quickviewprice"></li>
										<li class="list-inline-item text-muted font-weight-light">
											<del>$90.00</del>
										</li>
									</ul>
								</div>
								<p class="mb-4 text-muted" id="quickviewdescription"></p>
