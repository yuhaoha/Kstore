					...
					<table
						...
						<thead class="bg-light">
							<tr>
								...
								<th class="py-4 text-uppercase text-sm">日期</th>
								<th class="py-4 text-uppercase text-sm">总计</th>
								<th class="py-4 text-uppercase text-sm">状态</th>
								<th class="py-4 text-uppercase text-sm">查看</th>
								<th class="py-4 text-uppercase text-sm">收货</th>
							</tr>
						</thead>
						<tbody>
							<c:if test="${!empty ordersgoods}">
								<c:forEach items="${ordersgoods}" var="ordersgoods">
									<tr>
										<th class="py-4 align-middle">${ordersgoods.orders_id}</th>
										<td class="py-4 align-middle">${ordersgoods.buy_time}</td>
										<td class="py-4 align-middle">${ordersgoods.goods_price}
											RMB</td>
										<c:if test="${ordersgoods.state=='等待送达'}">
											<td class="py-4 align-middle"><span
												class="badge text-uppercase badge-info p-2">${ordersgoods.state}</span></td>
										</c:if>
										<c:if test="${ordersgoods.state=='已完成'}">
											<td class="py-4 align-middle"><span
												class="badge text-uppercase badge-success p-2">${ordersgoods.state}</span></td>
										</c:if>
										<td class="py-4 align-middle"><a
											href="order?order_id=${ordersgoods.orders_id}"
											class="btn btn-outline-dark btn-sm">查看</a></td>
										<td class="py-4 align-middle"><a
											href="confirmById?order_id=${ordersgoods.orders_id}"
											class="btn btn-outline-dark btn-sm">确认收货</a></td>
									</tr>
								</c:forEach>
							</c:if>
						</tbody>
				...
				<div class="col-xl-3 col-lg-4 mb-5">
					...
					: request.getAttribute("faith_valuemessage")%></span>
                      ...
                      <use xlink:href="#paper-bag-1"> </use>
								...
								<div
									class="badge badge-pill badge-light font-weight-normal px-3">${orderscount}</div></a>
